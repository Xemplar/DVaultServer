...
public class CryptoLinkRPCImpl implements CryptoLinkRPC {
	...
	@Override
	public String createRawTransaction(List<UTXOverview> outputs,
	...
	@Override
	public List<UTXOverview> listLockUnspent() throws CryptocoinException, CommunicationException {
		String lockedOutputsJson = rpcClient.execute(Commands.LIST_LOCK_UNSPENT.getName());
		List<UTXOverview> lockedOutputs = rpcClient.getMapper().mapToList(lockedOutputsJson,
				UTXOverview.class);
	...
	@Override
	public List<UTXO> listUnspent() throws CryptocoinException, CommunicationException {
		String unspentOutputsJson = rpcClient.execute(Commands.LIST_UNSPENT.getName());
		List<UTXO> unspentUTXOS = rpcClient.getMapper().mapToList(unspentOutputsJson,
				UTXO.class);
		return unspentUTXOS;
	}

	@Override
	public List<UTXO> listUnspent(Integer minConfirmations) throws CryptocoinException,
			CommunicationException {
		String unspentOutputsJson = rpcClient.execute(Commands.LIST_UNSPENT.getName(), 
				minConfirmations);
		List<UTXO> unspentUTXOS = rpcClient.getMapper().mapToList(unspentOutputsJson,
	...
	@Override
	public List<UTXO> listUnspent(Integer minConfirmations, Integer maxConfirmations)
			throws CryptocoinException, CommunicationException {
	...
	@Override
	public Boolean lockUnspent(Boolean isUnlocked, List<UTXOverview> outputs)
			throws CryptocoinException, CommunicationException {
	...
	@Override
	public String sendToAddress(String toAddress, BigDecimal amount, String comment,
								String commentTo, String narration) throws CryptocoinException, CommunicationException {
		amount = amount.setScale(Defaults.DECIMAL_SCALE, Defaults.ROUNDING_MODE);
		List<Object> params = CollectionUtils.asList(toAddress, amount, comment, commentTo, narration);
		String transactionIdJson = rpcClient.execute(Commands.SEND_TO_ADDRESS.getName(), params);
		String transactionId = rpcClient.getParser().parseString(transactionIdJson);
		return transactionId;
	}
	...
	@Override
	public String sendToAddress(String toAddress, BigDecimal amount, String narration, boolean type)
			throws CryptocoinException, CommunicationException {
		amount = amount.setScale(Defaults.DECIMAL_SCALE, Defaults.ROUNDING_MODE);
		List<Object> params = CollectionUtils.asList(toAddress, amount, "", "", narration);
		String transactionIdJson = rpcClient.execute(Commands.SEND_TO_ADDRESS.getName(), params);
		String transactionId = rpcClient.getParser().parseString(transactionIdJson);
		return transactionId;
	}
	...
	@Override
	public SignatureResult signRawTransaction(String hexTransaction, List<UTXO> utxos)
			throws CryptocoinException, CommunicationException {
		List<Object> params = CollectionUtils.asList(hexTransaction, utxos);
		String signatureResultJson = rpcClient.execute(Commands.SIGN_RAW_TRANSACTION.getName(), 
				params);
		SignatureResult signatureResult = rpcClient.getMapper().mapToEntity(signatureResultJson, 
				SignatureResult.class);
	...
	public SignatureResult signRawTransaction(String hexTransaction, List<UTXO> utxos,
		...
		List<Object> params = CollectionUtils.asList(hexTransaction, utxos, privateKeys);
		String signatureResultJson = rpcClient.execute(Commands.SIGN_RAW_TRANSACTION.getName(),
				params);
		SignatureResult signatureResult = rpcClient.getMapper().mapToEntity(signatureResultJson, 
				SignatureResult.class);
