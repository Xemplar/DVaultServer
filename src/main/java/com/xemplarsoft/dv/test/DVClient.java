...
public class DVClient implements Runnable{
    ...
    public DVClient(DVClientListener listener){
        ...
        byte[] decoded = java.util.Base64.getDecoder().decode("jsHjwrDrUnMlG6y+5qdsqVzntp+trHOPF5GXLui+5f0=");
        DVClient.this.sync = new SecretKeySpec(decoded, 0, decoded.length, "AES");
        DVClient.this.UID = 5375409349650691407L;
    }

    public DVClient(DVClientListener listener, String aes, long uid){
        this.listener = listener;
        t = new Thread(this);
        t2 = new Thread(this.actionListener);
        t3 = new Thread(this.reconnect);

        byte[] decoded = java.util.Base64.getDecoder().decode(aes);
        DVClient.this.sync = new SecretKeySpec(decoded, 0, decoded.length, "AES");
        DVClient.this.UID = uid;
    }
    ...
    public void registerOnNetwork(int step, String response){
        if(step == 0){
            ...
            chall = com.xemplarsoft.dv.medium.CryptoHandler.generateRandomString(20);
            listener.dwEventHappened("register attempt " + chall);
            String message = "loadmii:";
            sync = com.xemplarsoft.dv.medium.CryptoHandler.generateAES();
            String syncDat = "";
            try {
                syncDat = com.xemplarsoft.dv.medium.CryptoHandler.encryptMessage(new String(Base64.encode(sync.getEncoded())) + ":chall" + chall, async);
            ...
            try {
                ...
                String dat = com.xemplarsoft.dv.medium.CryptoHandler.decryptMessage(response, sync);
                if(dat.equals("chall" + chall)){
                    System.out.println("REGISTER: Challenge Correct");
                    listener.dwEventHappened("register correct");
                    write(com.xemplarsoft.dv.medium.CryptoHandler.encryptMessage("finishmii", sync));
                } else {
    ...
    public synchronized void encryptAndWrite(String message){
        try {
            String str = com.xemplarsoft.dv.medium.CryptoHandler.encryptMessage(message, sync);
    ...
    public void loginToNetwork(int step, String response){
        if(step == 0){
            ...
            try {
                ...
                String random = com.xemplarsoft.dv.medium.CryptoHandler.decryptMessage(dat[1], sync);
                ...
                write("verify:" + com.xemplarsoft.dv.medium.CryptoHandler.encryptMessage(modnar, sync));
    ...
    public void processMessage(String message){
        ...
        if(dat.length == 1){
            try{
                String data = com.xemplarsoft.dv.medium.CryptoHandler.decryptMessage(message, sync);
